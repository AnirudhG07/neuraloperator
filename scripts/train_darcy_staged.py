"""
Training script for Darcy flow equation using staged Fourier layers.
...
from config.darcy_staged_config import Default, Heavy

# Select config from CLI if provided, default to Default
config_classes = {"Default": Default, "Heavy": Heavy}
config_name = "Default"
for i, arg in enumerate(sys.argv):
    if arg == "--config" and i + 1 < len(sys.argv):
        config_name = sys.argv[i+1]
        # Remove these from sys.argv so zencfg doesn't get confused
        sys.argv.pop(i)
        sys.argv.pop(i)
        break

if config_name not in config_classes:
    print(f"Unknown config: {config_name}. Available: {list(config_classes.keys())}")
    sys.exit(1)

config = make_config_from_cli(config_classes[config_name])
