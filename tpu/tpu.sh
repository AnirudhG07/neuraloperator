...
#   • uploads THIS repo's tpu/ tree to ~/nop on the VM  (so `import tpu.profiling.bench_jax`)
...
BENCH_DEFAULT="export PATH=\$HOME/.local/bin:\$PATH; PYTHONPATH=\$HOME/$REMOTE_DIR ~/nopenv/bin/python -c \"import tpu.profiling.bench_jax as bj; bj.compare(case='rfft_1D', Ns=(256,1024,4096,8192,16384))\""
...
cmd_for() {
  ...
  --exclude=tpu_prof\\* --exclude='*.zip' --exclude=__pycache__ --exclude='*.png' --exclude=.venv \\
  $(basename "$LOCAL_DIR") && \\
...
gcloud compute tpus tpu-vm ssh $VM_NAME --zone=$ZONE --project=$PROJECT --worker=0 \\
  --command='rm -rf ~/$REMOTE_DIR && mkdir -p ~/$REMOTE_DIR && tar xzf ~/nop_tpu_upload.tar.gz -C ~/$REMOTE_DIR && ls ~/$REMOTE_DIR/tpu/*.py | head'" ;;
...
gcloud compute tpus tpu-vm scp $VM_NAME:/tmp/nop_prof.tar.gz $LOCAL_DIR/profiling/ \\
  --zone=$ZONE --project=$PROJECT --worker=0" ;;
...
note_for() {
  case "$1" in
    ...
    copyback)  echo "Tars /tmp/nop_prof (the profile() xplane traces) on the VM and pulls it into tpu/profiling/. Parse with bj.report() on a box that HAS TensorFlow." ;;
...
MENU=(
  ...
  $'copyback\tcopyback   pull profile traces back to tpu/profiling/'
