...
Compares the FNO forward at every fusion level, SAME math (same weights), NS size:
  baseline : plain-JAX FNO (fno.model.forward)           — XLA fuses what it can, inter-layer HBM
...
from ...fno.layers import init_fno
from ..fno.pallas_fused import forward_A, forward_B, forward_C

print("jax", jax.__version__, "| devices:", jax.devices(), flush=True)
WIDTH, M, LAYERS, H, W = 32, 16, 4, 128, 128
P = init_fno(jax.random.PRNGKey(1), 1, 1, WIDTH, M, LAYERS)
...
levels = {"baseline": lambda P, z, mdt: PR.forward(P, z, M),
...
for B in (1, 32):
    ...
    ref = jax.jit(lambda z: PR.forward(P, z, M))(x)
